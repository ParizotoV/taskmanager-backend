...
export class TaskService {
  ...
  async createTask(input: CreateTaskInputDto, user: CurrentUserData): Promise<Task> {
  ...
  async listTask(input: ListTaskInputDto, user: CurrentUserData): Promise<PaginatedTasksOutputDto<Task>> {
    return await this.listTaskUseCase.execute(input, user);
  }

  async updateTask(taskId: string, input: UpdateTaskInputDto, user: CurrentUserData): Promise<Task> {
    return await this.updateTaskUseCase.execute(taskId, input, user);
  }

  async updateStatusTask(taskId: string, status: TaskStatus, user: CurrentUserData, order?: number): Promise<Task> {
    return await this.updateStatusTaskUseCase.execute(taskId, status, user, order);
  }
}
...
import { UpdateTaskValidationError, TaskNotFoundError, TaskOwnershipError } from '@/application/task/errors/task.errors';
  ...
  async execute(
    ...
    try {
      ...
      if (task.userId !== user.id) {
        throw new TaskOwnershipError('Você só pode atualizar suas próprias tarefas');
      }

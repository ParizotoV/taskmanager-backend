...
import { UpdateStatusTaskValidationError, TaskNotFoundError, TaskOwnershipError } from '@/application/task/errors/task.errors';
  ...
  async execute(
    ...
    try {
      ...
      if (task.userId !== user.id) {
        throw new TaskOwnershipError('Você só pode atualizar o status de suas próprias tarefas');
      }
